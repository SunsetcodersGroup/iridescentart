 ...
 * @version         1.1.3               2016-12-30 15:38:50 SM:  Changed silly method for checking for start of a dockblock.
 * @version         1.1.4               2016-12-30 16:05:49 SM:  Trims the description (we tended to get prepended spaces etc).
 * @version         1.1.5               2016-12-30 16:11:19 SM:  Fixed bug where comments outside the main file block got nonsensically appended to the description field.
 */

namespace Prometheus\Vendors\FileAttributeTools;

// SM:  Testing only.
/*
$objFile=new FileAttributes(__FILE__);
echo "<p>File Attributes.</p>";
echo "<pre>";
print_r($objFile);
echo "</pre>";
echo "<p>XML Document.</p>";
echo "<pre>";
print_r(htmlentities($objFile->ToXML()->saveXML()));
echo "</pre>";
*/
...
final class FileAttributes
{
    ...
    public function __construct($txtFile)
    {
        ...
        $blnFinishedWithFileDocblock = false;
        $lngMaxVersion = 0;
        ...
        while ($blnProcessing) {
            ...
            elseif ($txtLine === false) {
                $blnProcessing = false;
            } elseif (stripos($txtLine, '*/')) {
                $blnWeAreInDockblock = false;
                $blnFinishedWithFileDocblock = true;
            } elseif (stripos($txtLine, '{')) {
                $blnProcessing = false;
            }
            ...
            if ($blnWeAreInDockblock && $blnProcessing) {
                ...
                if (stripos($txtLine, '/**') === false) {
                    ...
                    if (stripos($txtLine, '*') && !stripos($txtLine, '@') && !$blnFinishedWithFileDocblock) {
                    ...
                    if (stripos($txtLine, '@author')) {
                        if (stripos($txtLine, '<')) {
                            ...
                            if ($lngStart >= 0) {
                                $this->arrAuthor[$txtAuthorKey] = trim(substr($txtLine, $lngStart + 7,
                                    ($lngEnd - $lngStart - 7)));
                            }
                        } else {
                            $lngStart = stripos($txtLine, '@author');
                            $lngKey = count($this->arrAuthor) + 1;
                            $this->arrAuthor[$lngKey] = str_replace('@author', '',
                                trim(substr($txtLine, $lngStart + 7)));
                        }
        ...
        if ($this->txtDescription === '') {
            ...
            $this->txtDescription = trim($this->txtDescription);
        }
